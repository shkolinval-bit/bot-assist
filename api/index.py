# Файл: api/index.py (ФИНАЛЬНАЯ РАБОЧАЯ ВЕРСИЯ v4.0)
...
async def get_db_setting(db_session, key: str, default: str) -> str:
    # Эта функция должна стать асинхронной для asyncpg
    # Для простоты пока оставляем синхронной, но для продакшена лучше переписать
    setting = db_session.query(Settings).filter_by(setting_key=key).first()
    return setting.setting_value if setting else default

async def set_db_setting(db_session, key: str, value: str):
    setting = db_session.query(Settings).filter_by(setting_key=key).first()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = SessionLocal()
    try:
        welcome_text = await get_db_setting(db, 'welcome_text', "🎉 Бот запущен и подключен к базе данных. Используйте /admin для настроек.")
    finally:
        db.close()
...
#... (остальные хендлеры без изменений, но для полноты ответа они тут будут)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if SessionLocal:
        db = SessionLocal()
        try: mod_threshold = float(await get_db_setting(db, 'mod_threshold', str(MOD_THRESHOLD_DEFAULT)))
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'admin_moderation':
        if SessionLocal is None:
            ...
            return
        db = SessionLocal()
        try: current_threshold = await get_db_setting(db, 'mod_threshold', str(MOD_THRESHOLD_DEFAULT))
        finally: await db.close()
...
async def handle_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == STATE_AWAITING_NEW_THRESHOLD:
        ...
        db = SessionLocal()
        try:
            await set_db_setting(db, 'mod_threshold', str(float_value))
            context.user_data['state'] = None
